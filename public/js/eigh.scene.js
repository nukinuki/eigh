...
/** scene 
 *  Everything for game scene
 */
eigh.scene = new function(){

	var imgdir = eigh.config.imgdir;
	...
	function cameraCloseup(state){
		...
		if(!state){
			...
			var lookatface = new THREE.Vector3(0,1,0);
			lookatface.add(eigh.currentGame.devil.Mesh.position);
			var cameraposition = camera.position.clone();
			cameraposition.y = 2; // Closer to the ground
			cameraposition.add(eigh.currentGame.devil.Mesh.position).setLength(5).add(eigh.currentGame.devil.Mesh.position);
			var c = camera.clone();
		...
		state.progress += eigh.getSPF() / timelength;
		...
		if(state.progress < 1) {
			eigh.Animations.start(function(){cameraCloseup(state)});
		}
	}
	...
	function aquiLoaded(geometry, materials){
		...
		$(document).trigger('eighSceneLoaded');
		$(document).trigger('eighAquiLoaded');
	}
	function cubeLoaded(geometry, materials){
		...
		$(document).trigger('eighSceneLoaded');
		$(document).trigger('eighCubeLoaded');
	}
	...
	this.init = function(){
		...
		Textures = new function(){
			...
			this.marbleboard = new THREE.ImageUtils.loadTexture(imgdir + 'board.jpg', new THREE.UVMapping(), function(){
				...
				$(document).trigger('eighSceneLoaded');
				$(document).trigger('eighTexturesLoaded');
			});
		}
		...
		Materials.dummy = new THREE.MeshPhongMaterial({color: 0xdd0000});
		Materials.thunderbolt = new THREE.MeshLambertMaterial({emissive: 0xa6e9ff, color: 0xa6e9ff, transparent: true, opacity: 0.9});

		Materials.board = new THREE.MeshFaceMaterial([grayboard, grayboard, marbleboard, grayboard, grayboard, grayboard]);

		Materials.diceframe = new THREE.SpriteMaterial({
	...
	this.ready = function(callback){
		...
		if(isready){
			...
			$(document).one('eighSceneLoaded', function(){
				self.ready(callback);
			});
		}
	}
	...
	this.getScreenXY = getScreenXY;
	this.cameraCloseup = cameraCloseup;
}

window.components.require(['config', 'constants'], eigh.scene.init);
